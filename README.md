...
A bot detection pipeline that uses semantic embeddings and an ensemble of XGBoost models to predict if you're-a-bot-or-if-you're-not!

## The Cool Parts

* **Semantic Variance (Embeddings):** I use `all-MiniLM-L6-v2` to turn tweets and user descriptions into vectors, then calculate the mean and standard deviation (for tweets) of these embeddings.
...
The model is still quite consistent though. The `Profit Score` below is the bot-or-not challenge's internal scoring rules based on the true positives, false positives, and false negatives. Below results are on folds of 178 users.
...
I used SHAP to look inside the XGBoost model and see exactly which features the model uses to convict a user.
