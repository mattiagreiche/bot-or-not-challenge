...
# this file is a simple XGBoost model, but it takes into account the bot-or-not challenge's reward system to calculate
# the optimal "threshold" for guessing whether a user is a bot (or not).
# first, I trained with k-fold CV to get an idea of which features are best (out of curiosity) and to find this best threshold.
# then, we actually train on the full train set, saving the model and the PCA we used to reduce the dimensionality of all
# the embeddings
...
def train_and_eval(X, y):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        if USE_EMBEDDINGS:
            ...
            # pca for Ttweets
            pca_tweets = PCA(n_components=10, random_state=0)
            ...
            pca_desc = PCA(n_components=10, random_state=0)
